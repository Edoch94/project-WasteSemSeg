...
from src import loading_data, utils
...
def run_training():
    ...
    if len(configs.config_Enet.cfg["TRAIN"]["GPU_ID"]) > 1:
        net = torch.nn.DataParallel(
            net, device_ids=configs.config_Enet.cfg["TRAIN"]["GPU_ID"]
        ).cuda()
    else:
        net = net.cuda()
    ...
    optimizer = optim.Adam(
        net.parameters(),
        lr=configs.config_Enet.cfg["TRAIN"]["LR"],
        weight_decay=configs.config_Enet.cfg["TRAIN"]["WEIGHT_DECAY"],
    )
    scheduler = StepLR(
        optimizer,
        step_size=configs.config_Enet.cfg["TRAIN"]["NUM_EPOCH_LR_DECAY"],
        gamma=configs.config_Enet.cfg["TRAIN"]["LR_DECAY"],
    )
    ...
    for epoch in range(configs.config_Enet.cfg["TRAIN"]["MAX_EPOCH"]):
        ...
        print(
            f"epoch: {epoch:04d}, train_loss: {train_loss:.4f}, val_loss: {val_loss:.4f}, train_time {train_time:.4f}, val_time {val_time:.4f}"
        )
...
def evaluate(val_loader, net, criterion):
    ...
    with torch.no_grad():
        for vi, data in enumerate(val_loader, 0):
            ...
            iou_ += utils.calculate_mean_iu(
                [outputs.squeeze_(1).data.cpu().numpy()], [labels.data.cpu().numpy()], 2
            )
